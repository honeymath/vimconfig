...
def handler(**data):
    print(f"THE PDF LATEX FUNCTION HAS BEEN CALLED!!!!! CONGRADS{data}", flush=True)
    file = data.get("file", None)
    line = data.get("line", None)
    print(f"file: {file}, line: {line}", flush=True) 
    path = os.path.join(os.path.dirname(__file__), '../static')
    print(f"Path to static: {path}", flush=True)
    os.makedirs(path, exist_ok=True)
    ### the following logic is to find out the main.tex in the above langauge.
    from pathlib import Path
    """Return absolute path to nearest ancestor's main.tex, or '' if none."""
    d = Path(file).resolve()
    print(f"Starting directory: {d}", flush=True)
    while True:
        target = d / "main.tex"
        if target.is_file():
            break
        if d.parent == d:
            ...
            break
        d = d.parent
    print(f"Found main.tex at: {target}", flush=True)
    if target is not None:
        target = str(target.resolve())
        dirname = os.path.dirname(target)
    else:
        raise Exception("No main.tex found in the path hierarchy.")

    #the following: run pdflatex on the taregt
    print(f"Running pdflatex on {target}", flush=True)

    print(' '.join(["pdflatex", target]), flush=True)
    return {}
